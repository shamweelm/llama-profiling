...
def modMarker(mod, fn_name, args):
    ...
    d = {}
    d["mod"] = mod.__name__
    if fn_name == "forward" and len(args) > 0 and hasattr(args[0], 'extra_repr'):
...
def add_wrapper(mod, fn_name):
    ...
    s = (
        hasattr(mod, "extra_repr")
        and (type(mod) is not torch.jit.ScriptModule)
    ...
    def foo(args, name):
        ...
        for arg in args:
            if isinstance(arg, torch.Tensor):
                ...
                if (len(arg) == 0) or isscalar(
                    arg[0]
                ):  # An empty sequence or a sequence of scalars
                    seq(arg, name)
